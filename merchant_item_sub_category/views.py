...
from .serializers import PublicMerchantItemSubCategorySerializer, MerchantItemSubCategorySerializer,\
    MerchantItemSubCategoryStatusSerializer

from .models import MerchantItemSubCategory


class MerchantItemSubCategoryViewSet(viewsets.ModelViewSet):
    queryset = MerchantItemSubCategory.objects.all()
    ...
    def get_queryset(self):
        if self.action in ['list']:
            ...
            result = self.queryset.filter(merchant__user=self.request.user).order_by("category__name", "display_order")
    ...
    def get_serializer_class(self):
        if self.action in ['list', 'retrieve', 'partial_update', 'update', 'delete']:
            serializer = MerchantItemSubCategorySerializer
            ...
            serializer = PublicMerchantItemSubCategorySerializer

        elif self.action in ['create']:
            serializer = MerchantItemSubCategorySerializer

        elif self.action in ['partial_update_self']:
            serializer = MerchantItemSubCategoryStatusSerializer
