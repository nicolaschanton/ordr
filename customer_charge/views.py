...
from .serializers import CustomerChargeSerializer

from .models import CustomerCharge


class CustomerChargeViewSet(viewsets.ModelViewSet):
    queryset = CustomerCharge.objects.all()
    ...
    def get_object(self):
        if self.action in ['retrieve', 'partial_update', 'update', 'delete']:
            ...
            result = self.queryset.filter(customer_card__customer__user=self.request.user, id=self.kwargs["pk"]).first()
        else:
            result = self.queryset.filter(id=self.kwargs["pk"]).first()
        return result
    ...
    def get_queryset(self):
        if self.action in ['list']:
            ...
            result = self.queryset.filter(customer_card__customer__user=self.request.user)
    ...
    def get_serializer_class(self):
        if self.action in ['list', 'retrieve', 'partial_update', 'update', 'delete']:
            serializer = CustomerChargeSerializer
