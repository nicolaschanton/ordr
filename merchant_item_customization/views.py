...
from .serializers import MainMerchantItemCustomizationSerializer, MerchantItemCustomizationStatusSerializer

from .models import MerchantItemCustomization


class MerchantItemCustomizationViewSet(viewsets.ModelViewSet):
    queryset = MerchantItemCustomization.objects.all().order_by("display_order")
    ...
    def get_object(self):
        if self.action in ['retrieve', 'partial_update', 'update', 'delete']:
            ...
            result = self.queryset.filter(item__merchant__user=self.request.user, id=self.kwargs["pk"]).first()
        else:
            result = self.queryset.filter(id=self.kwargs["pk"]).first()
        return result
    ...
    def get_serializer_class(self):
        if self.action in ['list', 'retrieve', 'partial_update', 'update', 'delete']:
            serializer = MainMerchantItemCustomizationSerializer
            ...
            serializer = MerchantItemCustomizationStatusSerializer
