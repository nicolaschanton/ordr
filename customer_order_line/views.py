...
import sys
from .serializers import MainCustomerOrderLineSerializer

from .models import CustomerOrderLine


class CustomerOrderLineViewSet(viewsets.ModelViewSet):
    queryset = CustomerOrderLine.objects.all()
    ...
    def get_object(self):
        if self.action in ['retrieve', 'partial_update', 'update', 'delete']:
            ...
            result = self.queryset.filter(order__customer__user=self.request.user, id=self.kwargs["pk"]).first()
        else:
            result = self.queryset.filter(id=self.kwargs["pk"]).first()
        return result
    ...
    def get_queryset(self):
        if self.action in ['list']:
            ...
            result = self.queryset.filter(order__customer__user=self.request.user)
    ...
    def get_serializer_class(self):
        if self.action in ['list', 'retrieve', 'partial_update', 'update', 'delete']:
            serializer = MainCustomerOrderLineSerializer
    ...
    def list_self(self, request):
        try:
            if self.request.query_params.get("customer_order_id"):
                page = self.paginate_queryset(self.get_queryset().filter(order__id=self.request.query_params.get("customer_order_id")))
            else:
