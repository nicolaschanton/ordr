...
from .serializers import MerchantChargeSerializer

from .models import MerchantCharge


class MerchantChargeViewSet(viewsets.ModelViewSet):
    queryset = MerchantCharge.objects.all()
    ...
    def get_object(self):
        if self.action in ['retrieve', 'partial_update', 'update', 'delete']:
            ...
            result = self.queryset.filter(merchant_bank_account__merchant__user=self.request.user, id=self.kwargs["pk"]).first()
        else:
            result = self.queryset.filter(id=self.kwargs["pk"]).first()
        return result
    ...
    def get_queryset(self):
        if self.action in ['list']:
            ...
            result = self.queryset.filter(merchant_bank_account__merchant__user=self.request.user)
    ...
    def get_serializer_class(self):
        if self.action in ['list', 'retrieve', 'partial_update', 'update', 'delete']:
            serializer = MerchantChargeSerializer
