...
from .serializers import MerchantItemCustomizationOptionUpdateStatusSerializer, \
    MainMerchantItemCustomizationOptionSerializer

from .models import MerchantItemCustomizationOption


class MerchantItemCustomizationOptionViewSet(viewsets.ModelViewSet):
    queryset = MerchantItemCustomizationOption.objects.all().order_by("display_order")
    ...
    def get_object(self):
        if self.action in ['retrieve', 'partial_update', 'update', 'delete']:
            ...
            result = self.queryset.filter(item_customization__item__merchant__user=self.request.user, id=self.kwargs["pk"]).first()
        else:
            result = self.queryset.filter(id=self.kwargs["pk"]).first()
        return result
    ...
    def get_queryset(self):
        if self.action in ['list']:
            ...
            result = self.queryset.filter(item__merchant__user=self.request.user)
    ...
    def get_serializer_class(self):
        if self.action in ['list', 'retrieve', 'partial_update', 'update', 'delete']:
            serializer = MainMerchantItemCustomizationOptionSerializer
            ...
            serializer = MerchantItemCustomizationOptionUpdateStatusSerializer
